/**
 * =============================================================================
 * Script 02: SO₂ (Sulfur Dioxide) Extraction from Sentinel-5P TROPOMI
 * =============================================================================
 * 
 * Product: COPERNICUS/S5P/OFFL/L3_SO2
 * Band: SO2_column_number_density
 * QA Filter: SO2_column_number_density_amf > 0.5
 * 
...
var EXPORT_FOLDER = 'Nepal_S5P_SO2';
var SCALE = 5000;
...
var computeZonalStats = function(image) {
  ...
  var stats = zoneList.map(function(zoneId) {
    ...
    return ee.Feature(null, {
      ...
      'SO2_mean': result.get('SO2_column_number_density_mean'),
      'SO2_median': result.get('SO2_column_number_density_median'),
      'SO2_stdDev': result.get('SO2_column_number_density_stdDev'),
      'SO2_count': result.get('SO2_column_number_density_count'),
    });
  });
...
// ----- VISUALIZATION -----
var so2Viz = {min: 0, max: 0.0005, palette: ['#F3E5F5', '#CE93D8', '#AB47BC', '#7B1FA2', '#4A148C']};
Map.centerObject(nepal, 7);
Map.addLayer(monthlyCollection.filter(ee.Filter.eq('year', 2024)).mean(), so2Viz, 'SO2 Annual Mean 2024');

// ----- EXPORTS -----
Export.table.toDrive({
  collection: allZonalStats,
  description: 'SO2_monthly_zonal_stats_physiographic',
  folder: EXPORT_FOLDER, fileNamePrefix: 'SO2_monthly_zonal_physiographic', fileFormat: 'CSV',
});

Export.table.toDrive({
  collection: allProvinceStats,
  description: 'SO2_monthly_zonal_stats_province',
  folder: EXPORT_FOLDER, fileNamePrefix: 'SO2_monthly_zonal_province', fileFormat: 'CSV',
});

for (var yr = 2019; yr <= 2026; yr++) {
  Export.image.toDrive({
    image: monthlyCollection.filter(ee.Filter.eq('year', yr)).mean(),
    description: 'SO2_annual_mean_' + yr, folder: EXPORT_FOLDER,
    fileNamePrefix: 'SO2_annual_mean_' + yr, region: nepalGeom,
    scale: SCALE, crs: 'EPSG:4326', maxPixels: 1e10,
  });
}

print('SO2 Monthly Collection Size:', monthlyCollection.size());
